...
-- brutal force
-- lookAndSee : List Char -> List Char
-- lookAndSee list =
--     let (c1::rest) = list | _ => []
--     in go 1 c1 [] rest
--     where
--         go : Int -> Char -> List Char -> List Char -> List Char
--         go count lastChar acc [] = acc ++ (unpack $ show count) ++ [lastChar]
--         go count lastChar acc (c::cs) =
--             if (c == lastChar) then go (count + 1) lastChar acc cs
--             else go 1 c (acc ++ (unpack $ show count) ++ [lastChar]) cs
        

atomicElements : List (String, String, List String)
atomicElements =
    [ ("H", "22", ["H"])
...
runGame : Nat -> List String -> Nat
runGame repeats init =
    let mapping = SortedMap.fromList atomicElements
        result = repeatN repeats (lookAndSee mapping) init
    in foldl
        (\acc, atom =>
            case lookup atom mapping of
                Nothing => acc
                Just (seq, _) => acc + length seq
        )
        Z
        result
    where
        lookAndSee : SortedMap String (String, List String) -> List String -> List String
        lookAndSee _ [] = []
        lookAndSee map (x::xs) =
            case lookup x map of
                Nothing => []
                Just (_, list) => list ++ lookAndSee map xs


main : IO ()
main = do
    putStrLn $ "Part One result: " ++ (show $ runGame 40 ["Fr"])
    putStrLn $ "Part Two result: " ++ (show $ runGame 50 ["Fr"])
