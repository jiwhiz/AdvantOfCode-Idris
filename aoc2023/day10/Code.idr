...
run filename = do
    ...
    let (l::lines) := lines content | _ => printLn "Error parse file"
        board := fromList $ parseBoard (length $ trim l) (l::lines)
        Just (steps, newBoard) := cal board | Nothing => printLn "Error calculate steps"
    putStrLn $ "Part One result: " ++ (show $ div steps 2)
    putStrLn $ "Part Two result: " ++ (show $ countTile newBoard)


main : IO ()
main = do
    run "aoc2023/day10/test1.txt"
    run "aoc2023/day10/test3.txt"
    run "aoc2023/day10/test5.txt"
    run "aoc2023/day10/test6.txt"
    run "aoc2023/day10/input.txt"
