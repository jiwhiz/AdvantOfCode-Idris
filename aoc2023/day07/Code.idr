...
sortVect {n = 0} xs = xs
sortVect {n = (S k)} xs =
    let (idx, max) := maxVect xs
    in max :: sortVect (deleteAt idx xs)
...
part1 : Hand -> Hand -> Ordering
part1 h1 h2 =
    if getType h1 == getType h2 then
        comparingCards cardStrength h1 h2
    else
        comparing getType h1 h2
    where
        getType : Hand -> HandType
        getType = matchType . sortVect . count


part2 : Hand -> Hand -> Ordering
part2 h1 h2 =
    if getType h1 == getType h2 then
        comparingCards cardStrength2 h1 h2
    else
        comparing getType h1 h2
    where
...
run filename = do
    ...
    let list := mapMaybe parseOneLine $ lines content
    putStrLn $ "Part One result: " ++ (show $ calculate part1 list)
    putStrLn $ "Part Two result: " ++ (show $ calculate part2 list)


main : IO ()
main = do
    run "aoc2023/day07/test.txt"
    run "aoc2023/day07/input.txt"
