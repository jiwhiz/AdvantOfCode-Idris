...
countBelow : {n,l:Nat} -> (ratio : Nat) -> (startCol : Nat) -> (rowDistance : Nat) -> Vect (startCol + l) Nat -> Vect n (Vect (startCol + l) Cell) -> Nat
...
cal : {n,m:Nat} -> (ratio : Nat) -> (colRatios : Vect m Nat) -> Vect n (Vect m Cell) -> Nat
cal _ _ [] = 0
cal ratio colRatios (curRow :: remainingRows) =
    scanRow 0 {prf=plusZeroLeftNeutral m} colRatios curRow + cal ratio colRatios remainingRows
    where
        ...
        scanRow {len=S k} pos {prf} (colRatio :: remainingRatios) (y :: ys) =
            case y of
                ...
                G => countRight colRatio remainingRatios ys  -- sum of lengths to all G nodes at right side
                    + countBelow {l=S k} ratio pos (if allSpace curRow then ratio else 1) (rewrite prf in colRatios) (rewrite prf in remainingRows)
...
run filename = do
    ...
    let (l::lines) := lines content | _ => printLn "Error parse file"
        board := fromList $ parseBoard (length $ l) (l::lines)
    putStrLn $ "Part One result: " ++ (show $ cal 2 (expandRatio 2 (transpose board)) board)
    putStrLn $ "Part Two result: " ++ (show $ cal 1000000 (expandRatio 1000000 (transpose board)) board)


main : IO ()
main = do
    run "aoc2023/day11/test.txt"
    run "aoc2023/day11/input.txt"
