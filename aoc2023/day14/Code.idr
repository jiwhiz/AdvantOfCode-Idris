...
run filename = do
    ...
    let board := unpack <$> lines content
    putStrLn $ "Part One result: " ++ (show . northLoad . rollNorth) board

    let (len ** (idx, bList)) := runSpin board
        Just finalBoard := getBoard $ runSpin board | Nothing => printLn "Error to get final board"
    putStrLn $ "Part Two result: " ++ (show . northLoad) finalBoard


main : IO ()
main = do
    run "aoc2023/day14/test.txt"
    run "aoc2023/day14/input.txt"
